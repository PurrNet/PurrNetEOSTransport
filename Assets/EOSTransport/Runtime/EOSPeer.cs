...
namespace PurrNet.EOSTransport
{
    public class EOSPeer : IDisposable
    {
        ...
        public EOSPeer(P2PInterface p2p, ProductUserId localUserId, ProductUserId remoteUserId, string socketName)
        {
            _p2p = p2p;
        ...
        void OnSendFragment(ByteData fragment)
        {
            ...
            {
                _sendFailed = true;
                UnityEngine.Debug.LogError($"[EOSPeer] SendPacket failed: {result}");
            }
        }
        ...
        public void FlushQueue()
        {
            while (_fragmentQueue.Count > 0)
            {
                ...
                if (result != Result.Success)
                    UnityEngine.Debug.LogError($"[EOSPeer] Queued fragment send failed: {result}");
            }
